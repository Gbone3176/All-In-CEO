...
class GameEngine:
    ...
    def reset_game(self):
        ...
        # 大指标（抽象维度）- 放在商业征程上面
        self.large_metrics = {
            '团队士气': 70,          # 1-100，团队整体氛围和积极性
            '市场前景': 40,          # 1-100，市场认可度和发展潜力
            '公司控制权': 100,       # 创始团队股权占比，百分比
            '经营健康度': 50,        # 综合财务健康评分，1-100
            '品牌影响力': 20         # 品牌知名度和行业影响力，1-100
        }
        
        # 小指标（具象可量化）- 放在选项决策下面
        # 股权结构
        self.founding_team_equity = 100  # 创始团队股权占比
        self.investor_equity = 0         # 投资方股权占比
        self.option_pool_equity = 0      # 期权池占比
        self.minimum_control_threshold = 30  # 最小控制权阈值，低于此值失去公司控制
        
        # 财务报表 - 更详细的财务指标
        self.monthly_revenue = 80000     # 营业收入（月）
        self.monthly_cost = 50000        # 营业成本（月）
        self.gross_profit = 30000        # 毛利润（月）
        self.sales_expense = 20000       # 销售费用（月）
        self.management_expense = 30000  # 管理费用（月）
        self.rnd_expense = 40000         # 研发费用（月）
        self.financial_expense = 5000    # 财务费用（月）
        self.depreciation = 10000        # 折旧摊销（月）
        self.operating_profit = -75000   # 营业利润（月）
        self.net_profit = -80000         # 净利润（月）
        self.cash_reserve = 500000       # 现金储备
        self.asset_liability_ratio = 10  # 资产负债率，百分比
        self.gross_margin = 37.5         # 毛利率，百分比
        self.net_margin = -100.0         # 净利率，百分比
        
        # 团队相关
        self.core_employee_turnover = 2  # 核心员工流失率，百分比
        self.option_pool_usage = 0       # 期权池使用率，百分比
        
        # 市场相关
        self.revenue_growth_rate = 15    # 营业收入增长率，百分比
        self.market_share = 3            # 市场份额，百分比
        self.customer_acquisition_cost = 500  # 客户获取成本
        self.brand_recognition = 20      # 品牌认知度，百分比
        ...
        self.history.append({
            ...
            'event': '公司成立',
            'decision': '初始状态',
            'large_metrics': self.large_metrics.copy(),
            'founding_team_equity': self.founding_team_equity,
            'cash_reserve': self.cash_reserve,
            'monthly_profit': self.net_profit,
            'market_share': self.market_share
        })
        
        # 初始化当前会议（确保make_decision方法调用时有current_meeting属性）
        self.current_meeting = self.get_next_meeting()
    
    def get_current_state(self):
        # 更新财务指标
        self._update_financial_metrics()
        # 计算大指标的实时值
        self._update_large_metrics()
        
        return {
            'large_metrics': self.large_metrics,
            ...
            'detailed_metrics': {
                '股权结构': {
                    '创始团队股权占比': f'{self.founding_team_equity}%',
                    '投资方股权占比': f'{self.investor_equity}%',
                    '期权池占比': f'{self.option_pool_equity}%',
                    '最小控制权阈值': f'{self.minimum_control_threshold}%'
                },
                '财务报表': {
                    '营业收入(月)': f'{self.monthly_revenue:,}元',
                    '营业成本(月)': f'{self.monthly_cost:,}元',
                    '毛利润(月)': f'{self.gross_profit:,}元',
                    '毛利率': f'{self.gross_margin:.1f}%',
                    '销售费用(月)': f'{self.sales_expense:,}元',
                    '管理费用(月)': f'{self.management_expense:,}元',
                    '研发费用(月)': f'{self.rnd_expense:,}元',
                    '财务费用(月)': f'{self.financial_expense:,}元',
                    '四费合计(月)': f'{self.sales_expense + self.management_expense + self.rnd_expense + self.financial_expense:,}元',
                    '折旧摊销(月)': f'{self.depreciation:,}元',
                    '营业利润(月)': f'{self.operating_profit:,}元',
                    '净利润(月)': f'{self.net_profit:,}元',
                    '净利率': f'{self.net_margin:.1f}%',
                    '现金储备': f'{self.cash_reserve:,}元',
                    '资产负债率': f'{self.asset_liability_ratio}%'
                },
                '团队相关': {
                    '核心员工流失率': f'{self.core_employee_turnover}%',
                    '期权池使用率': f'{self.option_pool_usage}%'
                },
                '市场相关': {
                    '营业收入增长率': f'{self.revenue_growth_rate}%',
                    '市场份额': f'{self.market_share}%',
                    '客户获取成本': f'{self.customer_acquisition_cost:,}元',
                    '品牌认知度': f'{self.brand_recognition}%'
                }
            }
        }
    ...
    def _update_financial_metrics(self):
        # 计算毛利润
        self.gross_profit = self.monthly_revenue - self.monthly_cost
        # 计算毛利率
        self.gross_margin = (self.gross_profit / self.monthly_revenue * 100) if self.monthly_revenue > 0 else 0
        # 计算营业利润
        operating_expenses = self.sales_expense + self.management_expense + self.rnd_expense + self.financial_expense + self.depreciation
        self.operating_profit = self.gross_profit - operating_expenses
        # 计算净利润（简化处理，假设无其他调整）
        self.net_profit = self.operating_profit
        # 计算净利率
        self.net_margin = (self.net_profit / self.monthly_revenue * 100) if self.monthly_revenue > 0 else 0
    
    def _update_large_metrics(self):
        # 更新公司控制权指标（直接映射创始团队股权）
        self.large_metrics['公司控制权'] = self.founding_team_equity
        
        # 更新经营健康度指标（综合财务指标）
        financial_health = 0
        # 基于盈利能力（25%权重）
        if self.net_profit >= 0:
            profit_score = min(100, (self.net_profit / 10000) * 10 + 50)  # 利润越高得分越高
        else:
            profit_score = max(0, 50 + (self.net_profit / 10000))  # 亏损越多得分越低
        
        # 基于现金储备（25%权重）
        monthly_burn = -self.net_profit if self.net_profit < 0 else 10000
        months_runway = self.cash_reserve / monthly_burn if monthly_burn > 0 else 24
        cash_score = min(100, months_runway * 5)  # 6个月储备=30分，12个月=60分，24个月=100分
        
        # 基于毛利率（25%权重）
        gross_margin_score = min(100, self.gross_margin)
        
        # 基于资产负债率（25%权重）
        if self.asset_liability_ratio <= 30:
            debt_score = 100
        elif self.asset_liability_ratio <= 50:
            debt_score = 80
        elif self.asset_liability_ratio <= 70:
            debt_score = 60
        else:
            debt_score = max(0, 80 - (self.asset_liability_ratio - 70) * 2)
        
        financial_health = profit_score * 0.25 + cash_score * 0.25 + gross_margin_score * 0.25 + debt_score * 0.25
        self.large_metrics['经营健康度'] = round(financial_health, 1)
        
        # 更新团队士气指标
        turnover_score = max(0, 100 - self.core_employee_turnover * 2)  # 流失率每增加1%，得分减2
        option_score = self.option_pool_usage * 2  # 期权池使用率每1%，得分加2
        self.large_metrics['团队士气'] = round(turnover_score * 0.6 + option_score * 0.4, 1)
        
        # 更新市场前景指标
        growth_score = min(100, self.revenue_growth_rate * 4)  # 25%增长率=100分
        market_score = self.market_share * 10  # 10%市场份额=100分
        self.large_metrics['市场前景'] = round(growth_score * 0.5 + market_score * 0.5, 1)
        
        # 更新品牌影响力指标
        brand_score = min(100, self.brand_recognition)
        market_presence = min(100, (self.market_share / 10) * 100)
        self.large_metrics['品牌影响力'] = round(brand_score * 0.6 + market_presence * 0.4, 1)
        ...
        meetings = [
            {
                ...
                'impactA': {'cash': 100, 'revenue': 5, 'growth_rate': 10, 'equity': {'investor': 15, 'founding_team': -15}},
            'impactB': {'cash': 80, 'revenue': 3, 'growth_rate': 5, 'equity': {'investor': 10, 'founding_team': -10}, 'employee_turnover': -5},
            ...
            },
            {
                ...
                'impactA': {'cash': -40, 'employee_turnover': -8, 'option_usage': 10, 'growth_rate': 15},
            'impactB': {'cash': -25, 'employee_turnover': -5, 'option_usage': 5, 'growth_rate': 8},
            ...
            },
            {
                ...
                'impactA': {'cash': -15, 'employee_turnover': -5, 'market_share': 5, 'revenue': 3},
            'impactB': {'cash': -10, 'employee_turnover': -3, 'market_share': 8, 'revenue': 2},
            ...
            },
            {
                ...
                'impactA': {'cash': -20, 'employee_turnover': 5, 'revenue': 8, 'growth_rate': 5},
            'impactB': {'cash': -15, 'employee_turnover': -8, 'revenue': 5, 'growth_rate': 10},
            ...
            },
            {
                ...
                'impactA': {'cash': -25, 'employee_turnover': -12, 'option_usage': 15, 'growth_rate': 8},
            'impactB': {'cash': 0, 'employee_turnover': 10, 'market_share': -5, 'growth_rate': -8},
    ...
    def make_decision(self, option):
        ...
        # 记录决策前的主要指标
        prev_cash = self.cash_reserve
        prev_profit = self.net_profit
        prev_founding_equity = self.founding_team_equity
        prev_market_share = self.market_share
        
        # 检查是否为融资或收购相关事件（只允许这些事件影响股权结构）
        is_equity_related_event = False
        dilemma = self.current_meeting['dilemma']
        equity_keywords = ['融资', '投资', '股权', '收购', 'IPO', '控制权', '上市前']
        for keyword in equity_keywords:
            if keyword in dilemma:
                is_equity_related_event = True
                break
        
        # 仅在股权相关事件中更新股权指标
        if is_equity_related_event and 'equity' in impact:
            equity_impact = impact['equity']
            if 'founding_team' in equity_impact:
                self.founding_team_equity = max(0, min(100, self.founding_team_equity + equity_impact['founding_team']))
            if 'investor' in equity_impact:
                self.investor_equity = max(0, min(100, self.investor_equity + equity_impact['investor']))
            if 'option_pool' in equity_impact:
                self.option_pool_equity = max(0, min(100, self.option_pool_equity + equity_impact['option_pool']))
        
        # 特殊处理一些明确会影响控制权的事件
        # 1. 投资方派驻董事事件
        if '创始人控制权与融资需求平衡' in dilemma:
            if option == 'A':
                # 同意投资方派驻董事，创始团队股权减少
                self.founding_team_equity = max(30, self.founding_team_equity - 15)
                self.investor_equity = min(70, self.investor_equity + 15)
        
        # 2. 股权分配调整决策
        elif '股权分配调整决策' in dilemma:
            if option == 'A':
                # 缩减员工期权池，投资方增持
                self.founding_team_equity = max(30, self.founding_team_equity - 5)
                self.investor_equity = min(70, self.investor_equity + 10)
                self.option_pool_equity = max(0, self.option_pool_equity - 5)
            else:
                # 引入新战略投资方
                self.founding_team_equity = max(30, self.founding_team_equity - 10)
                self.investor_equity = min(70, self.investor_equity + 10)
        ...
        # 3. 收购事件
            ...
            if option == 'A':
                # 接受收购，创始团队股权大幅减少或归零
                self.founding_team_equity = 0
                self.investor_equity = 100
        
        # 更新财务相关指标
        if 'cash' in impact:
            self.cash_reserve = max(0, self.cash_reserve + impact['cash'])
        if 'revenue' in impact:
            self.monthly_revenue = max(0, self.monthly_revenue + impact['revenue'])
        if 'cost' in impact:
            self.monthly_cost = max(0, self.monthly_cost + impact['cost'])
        if 'sales_expense' in impact:
            self.sales_expense = max(0, self.sales_expense + impact['sales_expense'])
        if 'management_expense' in impact:
            self.management_expense = max(0, self.management_expense + impact['management_expense'])
        if 'rnd_expense' in impact:
            self.rnd_expense = max(0, self.rnd_expense + impact['rnd_expense'])
        if 'financial_expense' in impact:
            self.financial_expense = max(0, self.financial_expense + impact['financial_expense'])
        if 'depreciation' in impact:
            self.depreciation = max(0, self.depreciation + impact['depreciation'])
        if 'asset_liability_ratio' in impact:
            self.asset_liability_ratio = max(0, min(100, self.asset_liability_ratio + impact['asset_liability_ratio']))
        
        # 更新团队相关指标
        if 'employee_turnover' in impact:
            self.core_employee_turnover = max(0, min(100, self.core_employee_turnover + impact['employee_turnover']))
        if 'option_usage' in impact:
            self.option_pool_usage = max(0, min(100, self.option_pool_usage + impact['option_usage']))
        
        # 更新市场相关指标
        if 'growth_rate' in impact:
            self.revenue_growth_rate = max(0, min(100, self.revenue_growth_rate + impact['growth_rate']))
        if 'market_share' in impact:
            self.market_share = max(0, min(100, self.market_share + impact['market_share']))
        ...
        # 添加历史记录
        self.history.append({
            'stage': self.story_stage - 0.2,
            'event': self.current_meeting['dilemma'],
            'decision': decision_text,
            'feedback': feedback,
            'large_metrics': self.large_metrics.copy(),
            'cash_reserve': self.cash_reserve,
            'cash_change': impact.get('cash', 0),
        ...
        # 2. 创始团队失去控制权（低于最小控制阈值）
        elif self.founding_team_equity < self.minimum_control_threshold:
            self.game_over = True
            self.ending_type = 'loss_of_control'
        # 3. 团队士气崩溃
        elif self.core_employee_turnover > 50:
            self.game_over = True
            self.ending_type = 'team_disband'
        # 4. 市场前景暗淡
        elif self.market_share < 1 and self.revenue_growth_rate < 5:
            self.game_over = True
            self.ending_type = 'market_exit'
        # 5. 财务健康度极低
        elif self.asset_liability_ratio > 80 and self.net_profit < 0:
            self.game_over = True
            self.ending_type = 'financial_crisis'
        # 检查是否达到最终阶段
        elif self.story_stage >= 5:
            self.game_over = True
        ...
        return {
            'feedback': feedback,
            'cash_change': impact.get('cash', 0),
            'profit_change': self.net_profit - prev_profit,
            'equity_change': self.founding_team_equity - prev_founding_equity,
            'market_share_change': self.market_share - prev_market_share,
    ...
    def determine_final_ending(self):
        ...
        # 获取大指标值
        control_score = self.large_metrics.get('公司控制权', 0)
        health_score = self.large_metrics.get('经营健康度', 0)
        morale_score = self.large_metrics.get('团队士气', 0)
        market_score = self.large_metrics.get('市场前景', 0)
        brand_score = self.large_metrics.get('品牌影响力', 0)
        
        # 计算综合得分（包含品牌影响力）
        avg_score = (control_score + health_score + morale_score + market_score + brand_score) / 5
        
        # 基于各维度和综合表现判断结局
        # 1. 超级成功 - 所有指标都健康
        if avg_score >= 90 and control_score >= 80 and health_score >= 80:
            self.ending_type = 'unicorn_success'  # 独角兽级成功
        # 2. 显著成功 - 综合表现优秀
        elif avg_score >= 75:
            self.ending_type = 'ipo_ready'  # IPO就绪
        # 3. 良好成功 - 核心业务健康
        elif avg_score >= 60 and health_score >= 70 and control_score >= 40:
            self.ending_type = 'profitable_scale'
        # 4. 适度成功 - 业务稳定
        elif avg_score >= 50:
            self.ending_type = 'sustainable_business'
        # 5. 生存挑战 - 勉强维持
        elif avg_score >= 40:
            self.ending_type = 'challenging_survival'
        # 6. 创业困境 - 多个维度问题
        elif avg_score >= 30:
            self.ending_type = 'startup_dilemma'
        # 7. 失败结局 - 严重问题
        else:
            self.ending_type = '创业失败'
    
    def get_ending_text(self):
        endings = {
            # 失败结局
            'bankruptcy': '【游戏结束】公司资金耗尽，宣布破产。作为CEO，你未能带领公司度过难关，投资者对你的领导能力失去信心。',
            'loss_of_control': '【游戏结束】创始团队股权低于30%，失去对公司的控制权。投资者接管决策，你被迫离开自己创立的公司。',
            'team_disband': '【游戏结束】团队士气崩溃，核心员工流失率超过50%。没有了优秀的团队，公司无法继续运营，最终被市场淘汰。',
            'market_exit': '【游戏结束】市场份额低于1%且增长率低于5%，公司被迫退出市场。产品缺乏竞争力，被竞品完全挤压。',
            'financial_crisis': '【游戏结束】资产负债率超过80%且持续亏损，公司陷入严重财务危机。银行拒绝贷款，供应商要求预付，最终无法维持运营。',
            '创业失败': '【游戏结束】创业以失败告终。多个维度指标严重不达标，公司未能找到合适的商业模式和市场定位。',
            
            # 成功结局
            'unicorn_success': '【超级胜利】恭喜！你的公司成为估值超过10亿的独角兽企业。创始团队保持绝对控制权，业务健康增长，团队凝聚力强，市场前景广阔。你被誉为行业变革者，成为创业神话。',
            'ipo_ready': '【IPO就绪】公司已准备好上市。各项指标表现优秀，财务健康，市场份额领先，团队稳定。投资者看好你的商业模式，投行主动接洽。',
            'profitable_scale': '【规模化盈利】公司实现规模化盈利。业务模式被验证可行，现金流健康，市场稳步扩张。创始团队保持足够控制权，核心员工稳定。',
            'sustainable_business': '【可持续经营】公司建立了可持续的商业模式。虽然增速不及预期，但业务稳定，盈利能力良好，团队和谐，市场地位稳固。',
            'challenging_survival': '【勉强生存】公司勉强维持运营。你需要重新评估战略方向，改善财务状况，提升团队士气，寻找新的增长点。',
            'startup_dilemma': '【创业困境】公司面临多重困境。创始团队股权被严重稀释，财务状况不佳，团队士气低落，市场竞争激烈。你需要做出艰难抉择，可能需要大幅调整业务方向。'
        }
...
def make_decision():
    ...
    # 获取决策前的指标值用于计算变化
    prev_revenue = game_engine.monthly_revenue
    prev_growth_rate = game_engine.revenue_growth_rate
    prev_turnover = game_engine.core_employee_turnover
    
    # 保存决策前的大指标值
    game_engine._update_large_metrics()
    prev_large_metrics = game_engine.large_metrics.copy()
    ...
    # 计算实际的指标变化
    revenue_change = game_engine.monthly_revenue - prev_revenue
    growth_rate_change = game_engine.revenue_growth_rate - prev_growth_rate
    employee_turnover_change = game_engine.core_employee_turnover - prev_turnover
    
    # 计算大指标变化
    control_change = game_engine.large_metrics.get('公司控制权', 0) - prev_large_metrics.get('公司控制权', 0)
    health_change = game_engine.large_metrics.get('经营健康度', 0) - prev_large_metrics.get('经营健康度', 0)
    morale_change = game_engine.large_metrics.get('团队士气', 0) - prev_large_metrics.get('团队士气', 0)
    market_change = game_engine.large_metrics.get('市场前景', 0) - prev_large_metrics.get('市场前景', 0)
    brand_change = game_engine.large_metrics.get('品牌影响力', 0) - prev_large_metrics.get('品牌影响力', 0)
    
    # 计算财务指标变化
    profit_change = game_engine.net_profit - result.get('profit_change', 0)
    cost_change = game_engine.monthly_cost if hasattr(game_engine, 'monthly_cost') else 0
    margin_rate = (profit_change / revenue_change * 100) if revenue_change > 0 else 0
    ...
    response = {
        ...
        'equity_change': result.get('equity_change', 0),
        'employee_turnover_change': employee_turnover_change,
        # 添加前端所需的大指标变化字段
